...
def process(filename, filedata, output_folder):
    ...
    for line in lines:
        ...
        for word in words:
            if word in word_idx:
                col = word_idx[word]
                if len(heap) < max_document_length:
                    heapq.heappush(heap, tfidf[0, col])
                else:
                    heapq.heappushpop(heap, tfidf[0, col])
            else:
        ...
        # count num words with min_val
        min_val = heapq.heappop(heap)
        min_count = 1
        while len(heap) > 0 and min_val == heapq.heappop(heap):
            min_count += 1
        ...
        for word in words:
            if word in word_idx:
                col = word_idx[word]
                if tfidf[0, col] > min_val:
                    important_words.append(word)
                elif min_count > 0 and tfidf[0, col] == min_val:
                    min_count -= 1
                    important_words.append(word)
